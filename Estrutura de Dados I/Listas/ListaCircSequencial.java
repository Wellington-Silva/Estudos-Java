...
public class ListaCircSequencial<T extends Comparable<T>> extends Lista<T> {
    // Atributos
    T[] lista;
    int qtdMax, qtd_Elements=0, finalLista;

    // Construtor
    public ListaCircSequencial(Class<T> dataType){
        super();
        int qtdMax = 10;
    ...
    public boolean contem(T elemento) throws Exception {
        for (int i = 0; i < qtd_Elements; i++) {
            if (lista[i].equals(elemento)) return true;
        }
        return false;
    }
}
